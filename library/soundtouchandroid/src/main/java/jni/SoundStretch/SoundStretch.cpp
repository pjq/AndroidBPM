...
// Processes the sound
void SoundStretch::process(SoundTouch *pSoundTouch, WavInFile *inFile, WavOutFile *outFile)
{
    int nSamples;
    int nChannels;
    int buffSizeSamples;
...
float SoundStretch::detectBPM(WavInFile *inFile, RunParameters *params)
{
    LOGI("detectBPM in");
    float bpmValue;
    int nChannels = (int)inFile->getNumChannels();
    int sampleRate = (int)inFile->getSampleRate();
    LOGI("detectBPM, nChannels = %d", nChannels);
//    BPMDetect bpm(nChannels, inFile->getSampleRate());
    if(nChannels == 0){
        nChannels = 1;
        LOGI("detectBPM, to avoid crash set nChannels = %d", nChannels);
    }

    if ( 0 == sampleRate){
        sampleRate = 44100;
        LOGI("detectBPM, to avoid crash set sampleRate = %d", sampleRate);
    }

    BPMDetect bpm(nChannels,sampleRate);
    SAMPLETYPE sampleBuffer[BUFF_SIZE];
    LOGI("detectBPM, nChannels = %d, sampleRate = %d", nChannels, sampleRate);
...
int SoundStretch::mainCall(const int nParams, const char * const paramStr[])
{
    WavInFile *inFile;
    WavOutFile *outFile;
    RunParameters *params;
    SoundTouch soundTouch;
    LOGI("SoundStretch::mainCall");

    fprintf(stderr, _helloText, SoundTouch::getVersionString());

    try 
    {
       LOGI(" Parse command line parameters");
        ...
        if (params->detectBPM == TRUE)
        {
            ...
            LOGI("detectBPM");
            float bpm = detectBPM(inFile, params);
            LOGI("detectBPM,bpm = %d", bpm);
            return (int)bpm;
        }
